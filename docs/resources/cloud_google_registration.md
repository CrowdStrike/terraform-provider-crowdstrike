...
- `existing_wif_pool_id` (String) The ID of an existing GCP Workload Identity Pool, owned by another registration under the same CID, to attach this registration to instead of creating a new pool. Only valid for project-scoped registrations with no real-time visibility, DSPM, or vulnerability scanning enabled
