...
func getSelfServiceIOAExclusion(
	ctx context.Context,
	...
	if err != nil {
		diags.Append(tferrors.NewDiagnosticFromAPIError(tferrors.Read, err, selfServiceIOAExclusionRequiredScopes))
		return nil, diags
	}

	exclusion, payloadDiags := extractSelfServiceIOAExclusionFromPayload(tferrors.Read, resp.GetPayload(), id)
...
func extractSelfServiceIOAExclusionFromPayload(
	...
	if diagErr := diagnosticFromSelfServiceIOAPayload(operation, payload.Errors); diagErr != nil {
		diags.Append(diagErr)
		return nil, diags
	}

	resource := findSelfServiceIOAExclusion(payload.Resources, id)
	if resource != nil {
		return resource, diags
	}

	if id != "" {
		diags.Append(tferrors.NewNotFoundError(fmt.Sprintf("self-service IOA exclusion %q was not found", id)))
		return nil, diags
	}
...
func findSelfServiceIOAExclusion(
...
func diagnosticFromSelfServiceIOAPayload(
	operation tferrors.Operation,
...
func diagnosticFromSelfServiceIOAQueryPayload(
	operation tferrors.Operation,
