...
func TestContentUpdatePolicyPrecedenceResourceModel_wrap(t *testing.T) {
	tests := []struct {
		name          string
		policies      []string
		expectError   bool
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			model := &contentUpdatePolicyPrecedenceResourceModel{}
			ctx := t.Context()

			diags := model.wrap(ctx, tt.policies)
			...
			if !diags.HasError() {
				...
				model.IDs.ElementsAs(ctx, &actualPolicies, false)
