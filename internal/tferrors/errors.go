...
// extractAPIErrorDetail reads error messages from the response body of a
...
// Typed responses need no extraction: their Error method already renders the
// payload readably, so callers fall back to err.Error() when this returns "".
