...
func CloudAzureTenantResourceSchema(ctx context.Context) schema.Schema {
	return schema.Schema{
		Attributes: map[string]schema.Attribute{
			...
			"event_hub_settings": schema.ListNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					Attributes: map[string]schema.Attribute{
						"consumer_group": schema.StringAttribute{
							Required: true,
						},
						"event_hub_id": schema.StringAttribute{
							Required: true,
						},
					...
					CustomType: EventHubSettingsType{
						ObjectType: types.ObjectType{
							AttrTypes: EventHubSettingsValue{}.AttributeTypes(ctx),
						},
					},
				},
				Optional: true,
			...
			"microsoft_graph_permission_ids_readonly": schema.BoolAttribute{
				Optional: true,
				Computed: true,
			},
...
type CloudAzureTenantModel struct {
	...
	MicrosoftGraphPermissionIdsReadonly types.Bool                `tfsdk:"microsoft_graph_permission_ids_readonly"`
...
var _ basetypes.ObjectTypable = EventHubSettingsType{}

type EventHubSettingsType struct {
	basetypes.ObjectType
}

func (t EventHubSettingsType) Equal(o attr.Type) bool {
	other, ok := o.(EventHubSettingsType)
...
func (t EventHubSettingsType) String() string {
	return "EventHubSettingsType"
}

func (t EventHubSettingsType) ValueFromObject(ctx context.Context, in basetypes.ObjectValue) (basetypes.ObjectValuable, diag.Diagnostics) {
	var diags diag.Diagnostics

	attributes := in.Attributes()

	consumerGroupAttribute, ok := attributes["consumer_group"]

	if !ok {
		diags.AddError(
			"Attribute Missing",
			`consumer_group is missing from object`)

		return nil, diags
	}

	consumerGroupVal, ok := consumerGroupAttribute.(basetypes.StringValue)

	if !ok {
		diags.AddError(
			"Attribute Wrong Type",
			fmt.Sprintf(`consumer_group expected to be basetypes.StringValue, was: %T`, consumerGroupAttribute))
	}

	eventHubIdAttribute, ok := attributes["event_hub_id"]

	if !ok {
		diags.AddError(
			"Attribute Missing",
			`event_hub_id is missing from object`)

		return nil, diags
	}

	eventHubIdVal, ok := eventHubIdAttribute.(basetypes.StringValue)

	if !ok {
		diags.AddError(
			"Attribute Wrong Type",
			fmt.Sprintf(`event_hub_id expected to be basetypes.StringValue, was: %T`, eventHubIdAttribute))
	}
	...
	return EventHubSettingsValue{
		ConsumerGroup: consumerGroupVal,
		EventHubId:    eventHubIdVal,
		Purpose:       purposeVal,
		state:         attr.ValueStateKnown,
	}, diags
}

func NewEventHubSettingsValueNull() EventHubSettingsValue {
	return EventHubSettingsValue{
		state: attr.ValueStateNull,
	}
}

func NewEventHubSettingsValueUnknown() EventHubSettingsValue {
	return EventHubSettingsValue{
		state: attr.ValueStateUnknown,
	}
}

func NewEventHubSettingsValue(attributeTypes map[string]attr.Type, attributes map[string]attr.Value) (EventHubSettingsValue, diag.Diagnostics) {
	var diags diag.Diagnostics
	...
	for name, attributeType := range attributeTypes {
		...
		if !ok {
			diags.AddError(
				"Missing EventHubSettingsValue Attribute Value",
				"While creating a EventHubSettingsValue value, a missing attribute value was detected. "+
					"A EventHubSettingsValue must contain values for all attributes, even if null or unknown. "+
					"This is always an issue with the provider and should be reported to the provider developers.\n\n"+
					fmt.Sprintf("EventHubSettingsValue Attribute Name (%s) Expected Type: %s", name, attributeType.String()),
			)
		...
		if !attributeType.Equal(attribute.Type(ctx)) {
			diags.AddError(
				"Invalid EventHubSettingsValue Attribute Type",
				"While creating a EventHubSettingsValue value, an invalid attribute value was detected. "+
					"A EventHubSettingsValue must use a matching attribute type for the value. "+
					"This is always an issue with the provider and should be reported to the provider developers.\n\n"+
					fmt.Sprintf("EventHubSettingsValue Attribute Name (%s) Expected Type: %s\n", name, attributeType.String())+
					fmt.Sprintf("EventHubSettingsValue Attribute Name (%s) Given Type: %s", name, attribute.Type(ctx)),
			)
		}
	}
	...
	for name := range attributes {
		...
		if !ok {
			diags.AddError(
				"Extra EventHubSettingsValue Attribute Value",
				"While creating a EventHubSettingsValue value, an extra attribute value was detected. "+
					"A EventHubSettingsValue must not contain values beyond the expected attribute types. "+
					"This is always an issue with the provider and should be reported to the provider developers.\n\n"+
					fmt.Sprintf("Extra EventHubSettingsValue Attribute Name: %s", name),
			)
		}
	}

	if diags.HasError() {
		return NewEventHubSettingsValueUnknown(), diags
	}
...
func NewEventHubSettingsValueMust(attributeTypes map[string]attr.Type, attributes map[string]attr.Value) EventHubSettingsValue {
	object, diags := NewEventHubSettingsValue(attributeTypes, attributes)
	...
	if diags.HasError() {
		...
		panic("NewEventHubSettingsValueMust received error(s): " + strings.Join(diagsStrings, "\n"))
	}

	return object
}

func (t EventHubSettingsType) ValueFromTerraform(ctx context.Context, in tftypes.Value) (attr.Value, error) {
	if in.Type() == nil {
		return NewEventHubSettingsValueNull(), nil
	}
	...
	if !in.IsKnown() {
		return NewEventHubSettingsValueUnknown(), nil
	}
	...
	return NewEventHubSettingsValueMust(EventHubSettingsValue{}.AttributeTypes(ctx), attributes), nil
}

func (t EventHubSettingsType) ValueType(ctx context.Context) attr.Value {
	return EventHubSettingsValue{}
}

var _ basetypes.ObjectValuable = EventHubSettingsValue{}

type EventHubSettingsValue struct {
	ConsumerGroup basetypes.StringValue `tfsdk:"consumer_group"`
	EventHubId    basetypes.StringValue `tfsdk:"event_hub_id"`
	Purpose       basetypes.StringValue `tfsdk:"purpose"`
	state         attr.ValueState
}

func (v EventHubSettingsValue) ToTerraformValue(ctx context.Context) (tftypes.Value, error) {
	attrTypes := make(map[string]tftypes.Type, 3)

	var val tftypes.Value
	var err error

	attrTypes["consumer_group"] = basetypes.StringType{}.TerraformType(ctx)
	attrTypes["event_hub_id"] = basetypes.StringType{}.TerraformType(ctx)
	...
	case attr.ValueStateKnown:
		...
		val, err = v.ConsumerGroup.ToTerraformValue(ctx)

		if err != nil {
			return tftypes.NewValue(objectType, tftypes.UnknownValue), err
		}

		vals["consumer_group"] = val

		val, err = v.EventHubId.ToTerraformValue(ctx)

		if err != nil {
			return tftypes.NewValue(objectType, tftypes.UnknownValue), err
		}

		vals["event_hub_id"] = val
...
func (v EventHubSettingsValue) IsNull() bool {
	return v.state == attr.ValueStateNull
}

func (v EventHubSettingsValue) IsUnknown() bool {
	return v.state == attr.ValueStateUnknown
}

func (v EventHubSettingsValue) String() string {
	return "EventHubSettingsValue"
}

func (v EventHubSettingsValue) ToObjectValue(ctx context.Context) (basetypes.ObjectValue, diag.Diagnostics) {
	var diags diag.Diagnostics

	attributeTypes := map[string]attr.Type{
		"consumer_group": basetypes.StringType{},
		"event_hub_id":   basetypes.StringType{},
	...
	objVal, diags := types.ObjectValue(
		attributeTypes,
		map[string]attr.Value{
			"consumer_group": v.ConsumerGroup,
			"event_hub_id":   v.EventHubId,
			"purpose":        v.Purpose,
		})
...
func (v EventHubSettingsValue) Equal(o attr.Value) bool {
	other, ok := o.(EventHubSettingsValue)
	...
	if !v.ConsumerGroup.Equal(other.ConsumerGroup) {
		return false
	}

	if !v.EventHubId.Equal(other.EventHubId) {
		return false
	}
...
func (v EventHubSettingsValue) Type(ctx context.Context) attr.Type {
	return EventHubSettingsType{
		basetypes.ObjectType{
			AttrTypes: v.AttributeTypes(ctx),
		},
...
func (v EventHubSettingsValue) AttributeTypes(ctx context.Context) map[string]attr.Type {
	return map[string]attr.Type{
		"consumer_group": basetypes.StringType{},
		"event_hub_id":   basetypes.StringType{},
		"purpose":        basetypes.StringType{},
