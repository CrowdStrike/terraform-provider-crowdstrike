...
func (m *defaultPreventionPolicyWindowsResourceModel) generatePreventionSettings(ctx context.Context) ([]*models.PreventionSettingReqV1, diag.Diagnostics) {
	...
	if !m.CloudMLSliderForPupAdwareCloudEndUserScans.IsNull() {
		var slider mlSlider
		diagsSlider := m.CloudMLSliderForPupAdwareCloudEndUserScans.As(ctx, &slider, basetypes.ObjectAsOptions{})
		diags.Append(diagsSlider...)
		if diags.HasError() {
			return preventionSettings, diags
		}
...
func (m *defaultPreventionPolicyWindowsResourceModel) assignPreventionSettings(
	...
	if slider, ok := mlSliderSettings["CloudMLSliderForPupAdwareCloudEndUserScans"]; ok {
		...
		m.CloudMLSliderForPupAdwareCloudEndUserScans = objValue
	}
